...
// Background compositing : un fond fixe et réutilisable par rareté.
// TODO: remplacer ces chemins par les vrais assets une fois exportés dans /public.
export const RARITY_BACKGROUND: Record<Rarity, string> = {
  common: "/card-backgrounds/common.png",
  rare: "/card-backgrounds/rare.png",
  epic: "/card-backgrounds/epic.png",
  legendary: "/card-backgrounds/legendary.png",
  secret: "/card-backgrounds/secret.png",
};

// Rim-light : filtre drop-shadow appliqué directement sur le <img> du portrait.
// Suit l'alpha réel du détourage.
export const RARITY_RIM_GLOW: Record<Rarity, string> = {
  common: "drop-shadow(0 2px 8px rgba(0,0,0,0.25))",
  rare: "drop-shadow(0 0 12px rgba(80,120,216,0.4))",
  epic: "drop-shadow(0 0 14px rgba(124,58,237,0.5)) drop-shadow(0 0 26px rgba(124,58,237,0.22))",
  legendary: "drop-shadow(0 0 16px rgba(232,182,90,0.55)) drop-shadow(0 0 30px rgba(232,182,90,0.28))",
  secret: "drop-shadow(0 0 14px rgba(255,105,180,0.6)) drop-shadow(0 0 28px rgba(139,92,246,0.3)) drop-shadow(0 0 42px rgba(77,232,255,0.2))",
};
...
// French key → ISO code + English label for display.
export const NATION_MAP: Record<string, { flag: string; label: string }> = {
  allemagne: { flag: "DE", label: "Germany" },
  france:    { flag: "FR", label: "France" },
  angleterre:{ flag: "GB", label: "England" },
  italie:    { flag: "IT", label: "Italy" },
  espagne:   { flag: "ES", label: "Spain" },
  bresil:    { flag: "BR", label: "Brazil" },
  japon:     { flag: "JP", label: "Japan" },
  argentine: { flag: "AR", label: "Argentina" },
};
