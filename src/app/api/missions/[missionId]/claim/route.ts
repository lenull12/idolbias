...
import { MISSIONS } from "@/lib/gameConfig";
...
export async function POST(
  ...
  const def = dailyDef || weeklyDef || staticDef;
  ...
  if (prog.missionsClaimed.includes(missionId))
  ...
  await db.batch([
    ...
    db.update(progression).set({
      missionsClaimed: [...prog.missionsClaimed, missionId],
      updatedAt: now,
    }).where(eq(progression.playerId, playerId)),
  ]);
